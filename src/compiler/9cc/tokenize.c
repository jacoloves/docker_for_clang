...
// Consume th current token if it matche op .
...
void expect(char *s) {
  if (!peek(s))
    error_tok(token, "expexted \"%s\"", s);
  token = token->next;
}

// Ensure tha the current token is TK_IDENT.
char *expect_ident(void) {
  if (token->kind != TK_IDENT)
    error_tok(token, "exepected an identifier");
...
static Token *read_int_literal(Token *cur, char *start) {
  ...
  // Read a binary, octal, decimal or hexdecimal number.
  int base;
...
// Tokenize `use_input` and returns new tokens.
Token *tokenize(void) {
  char *p = user_input;
  Token head = {};
  Token *cur = &head;
